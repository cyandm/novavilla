...
<section id="product-archive" class="flex flex-col gap-11 md:gap-16">
	<?php if ($is_main_archive) : ?>
		<?php
		$hero_image = get_field('product_archive_hero_image', $home_id);
		$hero_image_url = is_array($hero_image) ? ($hero_image['url'] ?? '') : $hero_image;
		$hero_title = get_field('product_archive_hero_title', $home_id) ?: __('ویلایــــــــــــــی مـــــــدرن سریع‌تر از آنچه تصور می‌کنید', 'novavilla');
		...
		?>
		<section class="flex flex-col lg:flex-row-reverse lg:items-center gap-3 lg:gap-7">
			<div class="relative w-full lg:w-[53%] shrink-0">
				...
				<?php if (!empty($hero_image_url)) : ?>
					<img src="<?php echo esc_url($hero_image_url); ?>" alt="<?php echo esc_attr($hero_title); ?>" class="relative z-10 w-full h-56 lg:h-[439px] object-cover" loading="eager" decoding="async" />
				<?php else : ?>
			...
			<div class="w-full lg:flex-1 flex flex-col gap-3">
				<div class="flex flex-col gap-3">
					<h1 class="text-2xl lg:text-4xl font-bold text-cynTextPrimary leading-9 lg:leading-14"><?php echo esc_html($hero_title); ?></h1>
		...
		<div class="flex flex-col md:grid md:grid-cols-4 gap-4">
			<aside
				...
				<div class="flex items-center justify-between gap-3 md:hidden">
					...
					<button type="button" modal-closer data-modal-name="product-filter-modal" class="flex size-10 shrink-0 items-center justify-center rounded-full bg-cynBorderHover text-cynBlack" aria-label="<?php esc_attr_e('بستن', 'novavilla'); ?>">
						<i class="size-[18px] flex items-center justify-center [&_svg]:size-full [&_svg]:stroke-[1.5]"><?php Icon::print('circle-delete-cross'); ?></i>
					</button>
				</div>
