...
<form role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>" class="<?php echo esc_attr($form_class); ?>">
    ...
    <div class="flex w-full items-center">
        ...
        <label for="<?php echo esc_attr($input_id); ?>" class="flex min-w-0 flex-1 cursor-pointer items-center gap-1 self-stretch rounded-xl border-2 border-[#30303033] bg-cynWhite py-1 pe-1.5 ps-2.5 transition-all duration-300 focus-within:border-cynBorderHover">
            ...
            <input id="<?php echo esc_attr($input_id); ?>" type="search" name="s" value="<?php echo esc_attr(get_search_query()); ?>" placeholder="<?php echo esc_attr($placeholder); ?>" class="min-w-0 flex-1 border-0 bg-transparent text-sm font-normal text-cynBlack outline-none placeholder:text-cynBlack/50">
