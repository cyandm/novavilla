...
<main id="search-page" class="flex flex-col gap-5 md:gap-8">

    <section class="container">
        <form id="search-form" method="get" action="<?php echo esc_url(home_url('/')); ?>" class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <label for="search-page-input" class="flex w-full items-center gap-2.5 rounded-xl border border-[#30303033] bg-cynWhite py-1 pe-1.5 ps-2.5 transition-all duration-300 focus-within:border-cynBorderHover md:w-[379px] md:shrink-0">
            ...
            <div class="flex items-center justify-between gap-6 border-b border-cynTextPrimary/25 pb-5 md:border-0 md:pb-0">
                    ...
                    <?php foreach ($search_filters as $filter) : ?>
                        <label for="<?php echo esc_attr($filter['id']); ?>" class="group flex items-center gap-1 cursor-pointer">
                            ...
                            <span class="relative flex size-6 shrink-0 items-center justify-center rounded-full border border-[#ccc] bg-cynWhite group-has-[:checked]:border-cynBorderHover">
                                <input class="peer hidden" type="radio" name="search-type" id="<?php echo esc_attr($filter['id']); ?>" value="<?php echo esc_attr($filter['value']); ?>" <?php checked($search_type, $filter['value']); ?>>
    ...
    <?php if ($has_search) : ?>
        <section class="container flex flex-col gap-5 md:gap-5">
            ...
            <?php if (have_posts()) : ?>
                ...
                <div class="flex flex-col gap-11 md:gap-16">
                    <?php if (!empty($product_ids)) : ?>
                        <div id="searchProductsWrapper" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
